...
if [[ "${WEBRTC_MUX_ENABLED,,}" == "true" || "${WEBRTC_MUX_ENABLED}" == "1" || "${WEBRTC_MUX_ENABLED,,}" == "yes" || "${WEBRTC_MUX_ENABLED,,}" == "on" ]]; then
    ...
    WEBRTC_WORKER_ICELITE="${CLOUD_GAME_WEBRTC_ICELITE:-true}"
else
    WEBRTC_BASE_PORT="${WEBRTC_WORKER_BASE_PORT:-${WEBRTC_BASE_PORT:-8640}}"
    WEBRTC_WORKER_ICELITE="${CLOUD_GAME_WEBRTC_ICELITE:-false}"
fi
...
start coordinator env \
    ...
    CLOUD_GAME_COORDINATOR_ORIGIN_USERWS="${CLOUD_GAME_COORDINATOR_ORIGIN_USERWS:-*}" \
    CLOUD_GAME_COORDINATOR_SERVER_CACHECONTROL=no-store \
    COORDINATOR_HOST="$COORDINATOR_HOST" \
    HUB_ADDR="$HUB_ADDR" \
    CLOUD_GAME_WORKER_MONITORING_METRICENABLED="${CLOUD_GAME_WORKER_MONITORING_METRICENABLED:-true}" \
    CLOUD_GAME_WORKER_MONITORING_URLPREFIX="${CLOUD_GAME_WORKER_MONITORING_URLPREFIX:-/worker}" \
    NDS_AUTO_SPAWN="$NDS_AUTO_SPAWN" \
    NDS_ALLOW_PRIVATE_REMOTE_URLS="${NDS_ALLOW_PRIVATE_REMOTE_URLS:-}" \
...
worker_env=(
    ...
    CLOUD_GAME_WORKER_MONITORING_METRICENABLED="${CLOUD_GAME_WORKER_MONITORING_METRICENABLED:-true}"
    CLOUD_GAME_WORKER_MONITORING_URLPREFIX="${CLOUD_GAME_WORKER_MONITORING_URLPREFIX:-/worker}"
    CLOUD_GAME_WEBRTC_ICELITE="$WEBRTC_WORKER_ICELITE"
...
Firefox/strict NAT fallback for /v1 NDS rooms: set NDS_TURN_URLS and NDS_TURN_SECRET.
Static config TURN fallback is also supported with WEBRTC_TURN_URLS, WEBRTC_TURN_USERNAME, and WEBRTC_TURN_CREDENTIAL.
